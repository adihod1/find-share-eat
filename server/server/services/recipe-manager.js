...
class RecipeManager {
    ...
    getAllRecipes = async (filters) => {
        ...
        //search
        if (filters.search) {
            options.where = {[Op.and]: [{recipeName: {[Op.like]: `%${filters.search}%`}}]}
        }

        if (filters.ingredients?.length) {
            options.where = options.where || {}
            options.where[Op.or] = options.where[Op.or] || []
            options.where[Op.or].push(...(await _getUniqueIngredients(filters.ingredients, this.getRecipeByIngredients.bind(this), 'ingredientName')))
        }

        if (filters.categories?.length) {
            options.where = options.where || {}
            options.where[Op.or] = options.where[Op.or] || []
            options.where[Op.or].push(...(await _getUniqueCategories(filters.categories, this.getRecipeByCategories.bind(this), 'categoryName')))
        }
